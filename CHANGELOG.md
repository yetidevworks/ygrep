...
### Fixed
- **Symlink targets were ignored for their ancestors' names** — the watcher judged a symlink's canonical target by every component of its absolute path, so a workspace under `/tmp`, `~/build` or any other directory sharing a name with the ignored-directory list had its symlinked directories silently left unwatched. The target is now judged the way the walk judges directories: by its path inside the workspace when it lives there, and by its own name when it does not

## [4.0.0] - 2026-07-27

### Added
- **Management TUI, and `ygrep` on its own opens it** — bare `ygrep` with no query and no subcommand used to print help. On a terminal it now opens the management dashboard: an Indexes panel with watch state, path, size, files, segments, semantic badge, last-indexed time, live changes-per-minute and the persisted watch flag; a Service panel with installed/running state, pid, uptime, how many indexes are watched and where the log lives; and an Activity panel tailing what the watchers are doing. `enter` watches for the session, `w` toggles the persisted flag, `i` re-indexes, `c` compacts, `R` removes, `o` opens the workspace, `1`-`4` sort, `/` filters, `S` opens the service menu, `t` opens a full-screen query-stats view (queries/sec sparkline, totals, top queries and workspaces, live tail), and `?` lists the lot. Nothing slow runs on the event loop — registry walks, compaction, removals and service control run on worker threads and report back on a status line, so a failed action reads as a message rather than tearing the terminal down. Scripts are unaffected: unless both stdin and stdout are terminals, bare `ygrep` still prints help, so a piped or redirected invocation never blocks waiting for a keypress. `ygrep dashboard` still opens the same screen
- **Background watch service** (`ygrep service install|uninstall|start|stop|restart|status|log`) — keeping indexes current meant leaving `ygrep watch` running in a terminal per workspace, and closing that terminal silently stopped it. `ygrep service install` registers a user-level LaunchAgent on macOS or a systemd user unit on Linux that runs at login and watches every index whose watch flag is on, in one process. It holds a single-instance lock, logs to `<data_dir>/logs/service.log` with a 5 MB cap and one old generation, re-reads the index registry every 30 seconds so a flag toggled from the CLI or the TUI takes effect without any IPC or restart, records a heartbeat, and stops its watchers and compacts what needs it on SIGTERM rather than exiting mid-write. `ygrep service log -f` follows the log; `ygrep service status` reports installed, running, pid and watch count. Re-run `install` after the binary moves so the service definition points at the new path
- **Persisted watch flags** (`ygrep indexes watch [<hash|path>] on|off`) — which workspaces are worth watching is now a property of the index rather than of whichever terminal you left open. The flag is stored in the index's own metadata, survives re-indexing and upgrades, is listed by `ygrep indexes list`, is what the background service reads, and is toggled from the TUI with `w`
- **Query telemetry, feeding the TUI stats view** — every search appends one JSON line to `<data_dir>/telemetry/queries.jsonl`: timestamp, the workspace's index hash, the query truncated to 200 characters, milliseconds, hit count and mode (literal, regex or hybrid). It is local only — no network, no identifier beyond the hash already naming the index directory — rotates at 5 MB keeping one generation, and is written best-effort so a search never fails or stalls over a log line. Turn it off with `[output] telemetry = false`
- **`indexer.writer_heap_mb`** — the Tantivy writer heap for a full build (default 50), previously hardcoded in six places. Watch and single-file indexers take Tantivy's 15 MB minimum instead, which the dashboard had been charging to every watched workspace at 50 MB apiece
- **`[service]` configuration** — `registry_rescan_secs` (default 30) and `log_max_size_mb` (default 5) for the background service

### Changed
- **Indexing walks and indexes in parallel, and decides "unchanged" from metadata it already has** — an incremental run read the head of every file twice, stat-ed it three times and canonicalized its path, all before the mtime comparison that usually said "unchanged". The walk now yields the size and mtime it reads once, the mtime comparison happens first, and whether a file is generated is decided from the content the indexer reads anyway rather than from a separate 64 KB read. The walk itself moved from `walkdir` to the `ignore` crate and runs on `indexer.threads` worker threads, which had been dead config; adding a document takes a shared writer lock now that only a commit needs it exclusively. A no-op `ygrep index` over an 8.5k-file tree drops from 615 ms to 249 ms and over a 44.8k-file, 350 MB tree from 3.45 s to 0.84 s; a cold build of the 8.5k-file tree drops from 892 ms to 433 ms. On a tree large enough to saturate Tantivy's own indexing threads the parallel walk costs rather than pays — 9% more build time and 12% more disk, because interleaving unrelated files spoils the doc store's 64 KB compression blocks — so `indexer.threads = 1` gives all of that back while keeping the incremental win
- **Dotfiles are indexed, and gitignore rules are read the way git reads them** — the walk skipped every hidden file, so `.github/workflows/*.yml`, `.gitignore`, `.editorconfig` and `.env*` were invisible to a search even though they are source like any other. They are indexed now, along with the `.github`, `.gitlab`, `.circleci`, `.devcontainer`, `.husky` and `.changeset` directories; every other dot-directory is still pruned, and an unrecognised hidden file is still left alone rather than sniffed. With `respect_gitignore` on, nested `.gitignore` files, `.git/info/exclude` and the global excludes file are honoured where only the workspace's own root `.gitignore` was before, and they apply in a checkout with no `.git` of its own. Which files are indexable had drifted into three copies — the walk and both watch loops — with the watch copies quietly skipping the binary and generated-file checks, so a watched workspace could pick up files a rebuild would drop; there is one implementation now
- **Punctuation-only queries are pre-filtered and scanned across cores** — a literal like `->` or `<<<` has no index terms to narrow on, so the whole doc store has to be decompressed. It is now split into contiguous document ranges across cores, with each range watching the running totals ahead of it and stopping as soon as the page is covered, which keeps a dense query as cheap as the serial walk that used to stop after the first block. `<<<` drops from 29 ms to 12 ms on the 8.5k-file tree and from 197 ms to 45 ms on the 44.8k-file one
- **The semantic vector graph is read on first use** — every workspace open read the whole HNSW graph from disk, so `ygrep status`, a regex search, `--text-only` and an indexing run all paid for a structure they never touched, and the dashboard paid again for every watched workspace on each sleep/wake cycle. Answering "is semantic search available?" now reads the length of the stored document id list instead of building the graph to ask, and a full re-index starts from an empty index rather than loading the one it is about to replace. A hybrid search also issues one batched query for its vector neighbours instead of up to a few hundred sequential term lookups
- **Tens of millions of allocations cut from a large build** — splitting a token into its camelCase and snake_case parts built a `Vec<char>` for the token and a `String` for every part, on every token of every indexed file. The parts are slices of the token itself now, and a token with no boundary in it allocates nothing at all. Removing a document from the vector index scanned every stored id to find it, so a branch switch dropping five thousand files from a two-hundred-thousand-vector index made a billion string comparisons; a slot map makes each removal a single lookup. Documents collected for embedding are truncated to the 4 KB that actually gets embedded as they are collected, instead of every file being held whole until the walk finishes, which is what keeps peak build memory level (506 MB against 504 MB on a 350 MB tree) despite the parallel walk holding more files in flight
- **Literal candidates are fetched in steps rather than always fifty deep** — a literal query used to pull fifty times the requested limit out of the ranking and read every one of them. The first pass now asks for five times the limit and only a page that came up short escalates, resuming where the last pass stopped rather than re-reading from the top. A query with fewer matches than the requested limit still pays for both passes: a two-word query with 84 matches on the 44.8k-file tree costs 138 ms at `-n 100` against 113 ms for the old fixed pass, while the same query at `-n 20`, which the first pool satisfies, costs 7 ms against 9 ms. Most of what is left is not the extra pass but the larger doc store the parallel walk produces: the same query against an index built with `indexer.threads = 1` takes 120 ms
- **The index registry moved into `ygrep-core`** — index collection, identifier resolution, the safe-delete guards and the display formatters lived in a CLI command module, so the dashboard and the service had to reach into it. They are `ygrep_core::registry` now, with the single index-directory resolution that the library and the CLI had been keeping separate copies of
- **The `[daemon]` config section was replaced by `[service]`** — it described a socket path, an idle timeout and a concurrency limit for a daemon that was never built. Config files still carrying a `[daemon]` section keep loading; the section is ignored

### Fixed
- **`-e` and `-p` filters threw away most of their own matches** — both were applied to the finished result page, after it had already been cut to the requested limit, so a query whose best-scoring documents were all the wrong extension could return nothing at all. On a 38,700-file tree `ygrep function -e md` returned 0 results with hundreds of matching Markdown files in the index, `handler -e yaml` returned 0, `config -p grav/system/src/` returned 6 and `class -p 'grav-plugin-*/classes/'` returned 1 — all four now return a full page of 100. Filters run while candidates are collected, so the limit counts matches that passed the filter, and the extension plus any complete directory name in a path pattern are pushed into the Tantivy query itself. Measured on the benchmark corpora at `-n 100`: `-e md config` returns 100 where 3.5.1 returned 20, `-p pages/ page` 100 against 52, `-e c malloc` 100 against 61, and `-p Marlin/ temperature` 100 against 89
- **Two processes could write the same index at once** — opening a workspace for writing deleted Tantivy's writer lockfile every time, so a second process could take the lock while the first still held it and two live `IndexWriter`s would write the same index; the rebuild path went further and removed the directory out from under a live watcher. The lock is left alone now and a busy lock is reported as contention, auto-refresh checks for a build already in flight before starting its own, and the macOS stale-`META_LOCK` workaround only clears the lockfile after an acquire has actually failed
- **A search against an index a watcher held reported an error instead of results** — refusing to steal the writer lock made contention visible, and three paths handled it badly. A search whose staleness refresh hit the lock exited with an error rather than returning the results already on disk; the watch manager reported a watcher stopped the instant it aborted the task, so the re-index or compaction waiting on that signal opened its writer while the old one was still being dropped; and a failed re-index was reported as a plain error, leaving the TUI counting the action as in flight forever. Searches now fall back to the copy on disk and say so, stopping a watcher waits for it to unwind, and a failed re-index is a distinct event the TUI and the service log both act on
- **A watched workspace under `~/build` or any dotted ancestor got no file events at all** — the watcher matched its hidden-file, ignored-directory and ignore-pattern rules against the absolute path, so a workspace stored below a directory named like a build output or beginning with a dot silently received nothing. The walk had been fixed for this; the watcher had not. Rules are matched relative to the watch root now, and the watcher follows the walk's own allowlist for dot-directories, so the dotfiles the walk started indexing are also kept up to date rather than indexed once and never again
- **Watch mode accumulated segments without bound** — a watch loop commits per batch with merging disabled and never compacted, so a workspace watched for days ended up with thousands of segments. Both the standalone watch and the manager now compact once the queue drains and the segment count passes `indexer.auto_compact_segments`
- **Symlinked directories churned on every incremental pass** — a tree reachable through two directory symlinks is indexed once, under one alias, and the parallel walk claimed a target the moment a worker reached one of its links, so which alias won depended on thread timing and changed from run to run. On a workspace with 662 symlinks every no-op `ygrep index` reported 256 files indexed and 256 removed and left a new segment behind, the segment count climbing 1 to 4 to 5 over three passes that should have done nothing. Links are collected by canonical target and walked in a later pass, keeping the lexicographically first alias, so the choice is the same on every run
- **Oversize files were read into memory before being rejected** — the file-size limit was enforced after the whole file had been read. It is checked against the stat the walk already carries
- **A momentary lock failure re-indexed everything** — the incremental pass built its map of already-indexed files from a reader it opened without retry, and returned an empty map on failure, which reads as "nothing is indexed" and re-indexed the whole tree
- **A `====` banner line could overflow the stack** — the tokenizer recursed once per leading punctuation character while skipping it, so twenty thousand `=` recursed twenty thousand deep. It loops now
- **The vector index leaked on every re-open** — the reloader the HNSW graph is read through was leaked to satisfy a borrow, so a long-running dashboard leaked one per watched workspace per sleep/wake cycle. It is owned by the index and dropped with it
- **`-p '**/tests/'` matched paths it should not have** — pushing a path pattern into the query needs each anchor to be a complete path segment, and `**/` looked like a separator without being one: it matches the empty string, so `**/tests/` also matched `contests/`, whose path carries no `tests` term to look up. A segment following `**` is no longer used as an anchor, so the pushdown stays a superset of what the pattern keeps
...
- **Self-refreshing indexes** — instead of telling you to run `ygrep index`, a search brings the index up to date itself. An index over a day old gets an incremental pass (mtime-based, so unchanged files cost nothing); an index left by an older ygrep is rebuilt outright, since an outdated format can return wrong results rather than merely dated ones. Both respect `--no-auto-index` and `search.auto_index`, and fall back to a printed note when the index directory isn't writable. A semantic index with an outdated format still asks, because rebuilding one re-embeds every file. Indexing triggered behind a search reports a single summary line rather than the full breakdown
- **`indexer.docstore_compression_level`** — tune the doc store zstd level (1-22, or 0 for LZ4). Higher levels build a smaller index more slowly; search latency is flat across every level, since a query decompresses only the blocks holding its results. Measured on a 5.1k-file project, compacted to one segment: level 0 gives a 26.6 MB index, 3 gives 22.3 MB, 6 gives 21.5 MB, 9 gives 21.3 MB and 12 gives 21.3 MB, with build times of 1.18s, 1.12s, 1.14s, 1.16s and 1.36s. Level 6 is the default as the knee of that curve
...
### Changed
- **Doc store compressed with zstd instead of LZ4** — stored file content is about half of an index, and zstd compresses code roughly 40% smaller than LZ4 at Tantivy's block granularity, with a larger 64 KB block size for a few points more. Decompression is ~19% slower (761 MB/s vs 938 MB/s measured), which a query never notices because it touches only a handful of blocks. Combined with the exclusion work, indexes are roughly half their previous size. Measured by building each index with both releases and compacting to one segment: 36.9 MB to 21.5 MB, 24.0 MB to 12.0 MB, 6.5 MB to 3.5 MB, 3.0 MB to 1.8 MB, and 869 KB to 675 KB across five real projects. Existing indexes stay readable and are rebuilt into the new format the next time you search or index (schema version 5 to 6)
